...
function AjaxCreateOrder(e) {
    ...
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            ...
            alert("문제가 발생했습니다. 다시 시도해주세요.(from createorder)");
        }
    });
    return order_id;
}
...
function AjaxStoreTransaction(e, order_id, amount, type) {
    ...
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            ...
            alert("문제가 발생했습니다. 다시 시도해보삼(from AjaxStoreTransaction)");
        }
    });
    return merchant_id;
}
...
function ImpTransaction(e, order_id,merchant_id, imp_id, amount) {
    ...
    request.fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 404) {
            ...
            alert("문제가 발생했습니다. 다시 시도해주세요.(ImpTranaction error)");
        }
    });
}